...
class LastGraphState {
  public:

    struct GSDeletedEdge {
      LastNodeId tonode, fromnode;
    ...
    vector<LastCloseLastTuple> *closetuples;
    vector<LastNodeId> nodesinpreorder;
    ...
    LastNodeLabel centerlabel;
    LastEdgeLabel bicenterlabel;
    int closecount;
    bool selfdone; // set by isnormal to store whether the original graph has been
                   ...
                   // have to go through all Lastlevels
    ...
    struct GSEdge {
      ...
      GSEdge ( int tonode, int postonode, LastEdgeLabel edgelabel, bool close = false ): tonode ( tonode ), postonode ( postonode ), edgelabel ( edgelabel ), cyclemark ( 0 ), close ( close ) { }
    };
    ...
    void makeState ( LastDatabaseTree *databasetree );
    void undoState ();
    void insertNode ( LastNodeLabel nodelabel, short unsigned int maxdegree );
    void deleteNode2 ();
    vector<GSNode> nodes;
    int edgessize;
    ...
    int normalizeSelf ();
    void init ();
    void insertStartNode ( LastNodeLabel nodelabel );
    void deleteStartNode ();
    void insertNode ( int from, LastEdgeLabel edgelabel, short unsigned int maxdegree );
    void deleteNode ();
    void insertEdge ( int from, int to, LastEdgeLabel edgelabel );
...
class GSWalk {
  public:
      ...
      GSWalk() : activating(0.0), hops(0), cutoff(0.0), adj_m_sing(0), adj_m_rank(0), adj_m_size(0) {
        to_nodes_ex.clear();
      }
