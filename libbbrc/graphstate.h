...
class BbrcGraphState {
  public:

    struct GSDeletedEdge {
      BbrcNodeId tonode, fromnode;
    ...
    vector<BbrcCloseBbrcTuple> *closetuples;
    vector<BbrcNodeId> nodesinpreorder;
    ...
    BbrcNodeLabel centerlabel;
    BbrcEdgeLabel bicenterlabel;
    int closecount;
    bool selfdone; // set by isnormal to store whether the original graph has been
                   ...
                   // have to go through all Bbrclevels
    ...
    struct GSEdge {
      ...
      GSEdge ( int tonode, int postonode, BbrcEdgeLabel edgelabel, bool close = false ): tonode ( tonode ), postonode ( postonode ), edgelabel ( edgelabel ), cyclemark ( 0 ), close ( close ) { }
    };
    ...
    void makeState ( BbrcDatabaseTree *databasetree );
    void undoState ();
    void insertNode ( BbrcNodeLabel nodelabel, short unsigned int maxdegree );
    void deleteNode2 ();
    vector<GSNode> nodes;
    int edgessize;
    ...
    int normalizeSelf ();
    void init ();
    void insertStartNode ( BbrcNodeLabel nodelabel );
    void deleteStartNode ();
    void insertNode ( int from, BbrcEdgeLabel edgelabel, short unsigned int maxdegree );
    void deleteNode ();
    void insertEdge ( int from, int to, BbrcEdgeLabel edgelabel );
    void deleteEdge ( int from, int to );
    void deleteEdge ( GSEdge &edge ); // pushes deleted edge on stack
    void reinsertEdge (); // reinserts last edge on the stack
    BbrcNodeId lastNode () const { return nodes.size () - 1; }
